...
UInput::UInput(const char *dev_name, const std::map<const __u16, std::list<__u16> > & keys) : fd(-1) {
	openAll();
	setup(dev_name, keys);
	create();
}
...
void UInput::setup(const char *dev_name, const std::map<const __u16,  std::list<__u16> > & keys) {
	...
	for (auto i = keys.begin(); i != keys.end(); ++i) {
		const std::list<__u16> & kk = i->second;
